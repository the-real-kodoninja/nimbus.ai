...
interface Props {
  isDarkTheme: boolean;
  userSettings: { aiName: string; voice: string };
}

interface PsychProfile {
  trait: string;
  description: string;
  thunderheadInsight: string;
  ghostState: string;
}
...
const UserProfile: React.FC<Props> = ({ isDarkTheme, userSettings }) => {
  ...
  const historyData: { query: string; response: string }[] = location.state?.history || [];
  const [userStats, setUserStats] = useState<UserStats>({ searches: 0, frequency: 0, visits: 0 });
  const [connectionLevel, setConnectionLevel] = useState<number>(75);
  ...
  useEffect(() => {
    ...
    setUserStats({ searches, frequency: parseFloat(frequency.toFixed(2)), visits });
  ...
  const generatePsychProfile = (history: { query: string }[]): PsychProfile => {
    const codeQueries = history.filter(item =>
      item.query.toLowerCase().includes('code') || item.query.toLowerCase().includes('program')
    ).length;
    const appQueries = history.filter(item =>
      item.query.toLowerCase().includes('app') || item.query.toLowerCase().includes('design')
    ).length;
      ...
      <Box sx={{ flexGrow: 1, padding: 4, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <Paper
          ...
          sx={{
            ...
            padding: 2,
            textAlign: 'center',
            background: 'linear-gradient(45deg, #1e1e1e 30%, #2e2e2e 90%)',
          }}
        >
          ...
          <Box sx={{ marginTop: 4 }}>
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>
              Frequency: {userStats.frequency} searches/day
            </Typography>
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>
              Visits: {userStats.visits}
            </Typography>
            <Typography variant="body2" sx={{ color: 'text.secondary', marginTop: 1 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 2 }}>
              ...
              <LinearProgress
                ...
                sx={{
                  ...
                  '& .MuiLinearProgress-bar': { backgroundColor: '#ce93d8' },
                }}
              />
